...
namespace REGoth
{
  ...
  {
    float elapsedSecondsOfHour =
        mElapsedIngameSeconds - (getDay() * SECONDS_IN_A_DAY) - (getHour() * SECONDS_IN_AN_HOUR);
  ...
  bool GameClock::isTime(bs::INT32 hour1, bs::INT32 min1, bs::INT32 hour2, bs::INT32 min2) const
  {
    ...
    {
      secondTimeOfDayInSeconds -=
          60.0;  // Gothic subtracts one minute from the second time here to counter issues happening
